...
namespace RIMAPI.Controllers
{
    public class ThingController
    {
        ...
        [Get("/api/v1/things/forbidden")]
        [EndpointMetadata("List forbidden, haulable things on a map")]
        public async Task GetForbiddenThings(HttpListenerContext context)
        {
            var mapId = RequestParser.GetMapId(context);
            var result = _thingsService.GetForbiddenThings(mapId);
            await context.SendJsonResponse(result);
        }
    }
}
