...
namespace RIMAPI.Controllers
{
    public class GlobalMapController : BaseController
    {
        ...
        {
            var body = await context.Request.ReadBodyAsync<StartTradeCaravanRequestDto>();
            var result = CaravanAutomationHelper.StartTradeCaravan(body);
            await context.SendJsonResponse(result);
        }

        [Get("/api/v1/world/trade/destinations")]
        public async Task GetTradeDestinations(HttpListenerContext context)
        {
            var mapId = RequestParser.GetIntParameter(context, "map_id");
            var result = CaravanAutomationHelper.GetTradeDestinations(mapId);
            await context.SendJsonResponse(result);
        }

        [Get("/api/v1/world/raid/destinations")]
        public async Task GetRaidDestinations(HttpListenerContext context)
        {
            var mapId = RequestParser.GetIntParameter(context, "map_id");
            var result = CaravanAutomationHelper.GetRaidDestinations(mapId);
            await context.SendJsonResponse(result);
        }

        [Post("/api/v1/world/caravan/raid/start")]
        public async Task StartRaidCaravan(HttpListenerContext context)
        {
            var body = await context.Request.ReadBodyAsync<StartRaidCaravanRequestDto>();
            var result = CaravanAutomationHelper.StartRaidCaravan(body);
            await context.SendJsonResponse(result);
        }
