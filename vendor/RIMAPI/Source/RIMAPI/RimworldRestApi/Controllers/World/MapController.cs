...
namespace RIMAPI.Controllers
{
    public class MapController
    {
        ...
        [Get("/api/v1/map/ancient-danger")]
        [EndpointMetadata("Get a legitimately detected Ancient Danger warning without exposing hidden contents")]
        public async Task GetAncientDanger(HttpListenerContext context)
        {
            var mapId = RequestParser.GetMapId(context);
            var result = AncientDangerAutomationHelper.GetState(mapId);
            await context.SendJsonResponse(result);
        }

        [Post("/api/v1/map/ancient-danger/open")]
        [EndpointMetadata("Designate the detected sealed Ancient Danger wall for ordinary deconstruction")]
        public async Task OpenAncientDanger(HttpListenerContext context)
        {
            var body = await context.Request.ReadBodyAsync<OpenAncientDangerRequestDto>();
            var result = AncientDangerAutomationHelper.Open(body);
            await context.SendJsonResponse(result);
        }

        [Post("/api/v1/map/plants/harvest")]
        [EndpointMetadata("Designate an explicit list of mature plants for harvesting")]
        public async Task DesignatePlantsForHarvest(HttpListenerContext context)
        {
            var body = await context.Request.ReadBodyAsync<HarvestPlantsRequestDto>();
            var result = MapHelper.DesignatePlantsForHarvest(body);
            await context.SendJsonResponse(result);
        }

        [Post("/api/v1/map/animal/tame")]
        [EndpointMetadata("Designate one specific wild animal for taming")]
        public async Task DesignateAnimalForTaming(HttpListenerContext context)
        {
            var body = await context.Request.ReadBodyAsync<TameAnimalRequestDto>();
            var result = MapHelper.DesignateAnimalForTaming(body);
            await context.SendJsonResponse(result);
        }
        ...
        {
            var body = await context.Request.ReadBodyAsync<HuntAnimalRequestDto>();
            var result = MapHelper.DesignateAnimalForHunting(body);
            await context.SendJsonResponse(result);
        }

        [Post("/api/v1/map/colonists/reproduction")]
        [EndpointMetadata("Set a romantic couple's pregnancy approach and assign a double bed when available")]
        public async Task SetHumanReproductionPlan(HttpListenerContext context)
        {
            var body = await context.Request.ReadBodyAsync<HumanReproductionRequestDto>();
            var result = MapHelper.SetHumanReproductionPlan(body);
            await context.SendJsonResponse(result);
        }
        ...
        [Post("/api/v1/map/zone/growing/sowing")]
        [EndpointMetadata("Enable or disable sowing in one existing growing zone without deleting crops")]
        public async Task SetGrowingSowing(HttpListenerContext context)
        {
            var body = await context.Request.ReadBodyAsync<SetGrowingSowingRequestDto>();
            var result = MapHelper.SetGrowingSowing(body);
            await context.SendJsonResponse(result);
        }
