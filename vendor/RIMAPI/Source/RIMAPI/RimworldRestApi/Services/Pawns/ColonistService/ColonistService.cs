    ...
    public class ColonistService : IColonistService
    {
        ...
        public ApiResult SetColonistWorkPriority(WorkPriorityRequestDto body)
        {
            try
            {
                ...
                // RimWorld manual work priorities use 0 (disabled) through 4.
                if (body.Priority < 0 || body.Priority > 4)
                {
                    return ApiResult.Fail($"Invalid priority {body.Priority}. Must be between 0 and 4");
                }

                // In the simple checkbox work mode RimWorld normalizes every enabled
                // work type to priority 3. Switch to the manual-priority mode before
                // applying an explicit API priority so the requested value persists.
