...
namespace RIMAPI.Services
{
    public class MapService : IMapService
    {
        ...
        public ApiResult<MapWeatherDto> GetWeather(int mapId)
        {
            ...
            Twelfth currentTwelfth = GenDate.Twelfth(GenTicks.TicksAbs, longitude);
            var result = new MapWeatherDto
            {
                Weather = map.weatherManager?.curWeather?.defName,
                Temperature = map.mapTemperature?.OutdoorTemp ?? 0f,
                GrowthSeasonNow = PlantUtility.GrowthSeasonNow(
                    map.Center, map, DefDatabase<ThingDef>.GetNamedSilentFail("Plant_Rice")),
                DayOfYear = GenDate.DayOfYear(GenTicks.TicksAbs, longitude) + 1,
                Quadrum = GenDate.Quadrum(GenTicks.TicksAbs, longitude).ToString(),
                CurrentTwelfth = currentTwelfth.ToString(),
                NextTwelfthAverageTemperatures = Enumerable.Range(0, 4)
                    .Select(offset => GenTemperature.AverageTemperatureAtTileForTwelfth(
                        map.Tile, (Twelfth)(((int)currentTwelfth + offset) % 12)))
                    .ToList(),
            };
