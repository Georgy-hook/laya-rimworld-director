...
namespace RIMAPI.Services
{
    class ThingsService : IThingsService
    {
        ...
        public ApiResult<List<ThingDto>> GetForbiddenThings(int mapId)
        {
            try
            {
                var map = Find.Maps.FirstOrDefault(m => m.uniqueID == mapId);
                if (map == null)
                    return ApiResult<List<ThingDto>>.Fail($"Map {mapId} not found");

                var things = map.listerThings.AllThings
                    .Where(t => t.Spawned && t.def.EverStorable(false) && t.IsForbidden(Faction.OfPlayer))
                    .Select(ThingDto.ToDto)
                    .ToList();
                return ApiResult<List<ThingDto>>.Ok(things);
            }
            catch (Exception ex)
            {
                return ApiResult<List<ThingDto>>.Fail($"Failed to list forbidden things: {ex.Message}");
            }
        }
