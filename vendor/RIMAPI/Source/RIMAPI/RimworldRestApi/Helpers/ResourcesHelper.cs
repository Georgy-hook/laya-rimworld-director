    ...
    public static class ResourcesHelper
    {
        ...
        public static ResourcesFoodSummaryDto GetResourcesFoodSummary(Map map, List<Thing> allItems)
        {
            ...
            var dto = new ResourcesFoodSummaryDto
            {
                ...
                TotalNutrition = foodItems.Sum(t =>
                    t.GetStatValue(StatDefOf.Nutrition) * t.stackCount
                ),
                MealsCount = foodItems
                    .Where(t => t.def.ingestible.IsMeal)
                    .Sum(t => t.stackCount),
                RawFoodCount = foodItems
                    .Where(t => !t.def.ingestible.IsMeal)
                    .Sum(t => t.stackCount),
                RotStatusInfo = GetFoodRotStatusInfo(map, foodItems),
            };
        ...
        public static ThingDto ThingToDto(Thing thing)
        {
            ...
            {
                dto.InnerDefName = minified.InnerThing.def.defName;
                dto.Beauty = minified.InnerThing.GetStatValue(StatDefOf.Beauty);
                if (minified.InnerThing.TryGetQuality(out QualityCategory innerQuality))
                    dto.InnerQuality = (int)innerQuality;
            }
